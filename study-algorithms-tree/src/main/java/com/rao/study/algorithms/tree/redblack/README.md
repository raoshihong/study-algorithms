...
,如果是右节点的最小子孙节点,那么这个最小子孙节点必定只有右子节点,所以此时他的替代节点为其右子节点
比如：
  ...
  根据上面的树查找替代节点
  第一中情况：删除8节点，那么替代节点可以理解为8
  第二种情况：删除节点为5，那么替代节点可以理解为8；或者删除15，那么替代节点可以理解为13
  第三种情况：删除节点为20，那么寻找20的右节点的最小子孙节点22,将节点22的值替换到节点20处，再删除节点22，所以这里实际删除的是22这个节点
  删除22节点满足第二种情况，所以根据第二种情况，找到替代节点为24
    ...
    2.如果替代节点为黑色且不是树的根节点，则需要根据替代节点的兄弟节点进行判断
     2.1替代节点为其父节点的左节点的情况
        ...
        2.1.2 替代节点的兄弟节点为红色，则将父节点设置为红色，兄弟节点设置为黑色，对父节点进行左旋，得到x对兄弟节点为黑色，那么又相当于再次对x为黑色，x的兄弟节点为黑色对情况进行判断兄弟的左右子节点的颜色判断
              红黑树如果一个节点为红色，那么它的父节点一定为黑色，它的左右子节点一定为黑色，所以经过2.1.2的情况处理，最终x的兄弟节点变成了黑色，所以又要重新对x进行上面对情况对处理
